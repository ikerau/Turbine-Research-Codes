...
def hydraulic_diam_blade(blade, gas):
...
def secondary_loss_KO(blade, inlet, outlet, gamma, M_exit, M_inlet):
    """
    Kacker-Okapuu (1982) secondary loss coefficient, Equations 11-16.
    Returns zeta (entropy loss coefficient).

    Sign convention (KO Fig. 3):
        alpha_1 : inlet  flow angle, positive magnitude [deg]
        alpha_2 : exit   flow angle, positive magnitude [deg]
        alpha_m : mean   flow angle per Eq. 13 — arctan[½(tan α₁ - tan α₂)]
                  where α₂ enters NEGATIVE (opposite side of axial to α₁)
                  ∴ equivalent to arctan[½(tan α₁ + |tan α₂|)]
        beta_1k : inlet metal angle (= Beta_1 at zero incidence)
    """
    # ── Geometry ──────────────────────────────────────────────────────────────
    bx = blade["Chord_ax"].to('m').magnitude
    c   = float(blade["Chord_ax"].to('m').magnitude) #\
          #/ np.cos(blade["stagger"].to('rad').magnitude)          # true chord
    h   = float((blade["R_o_outlet"] - blade["R_i_outlet"]).to('m').magnitude)           # aerodynamic span
    Rm  = mean_radius(blade, outlet=True)
    s   = float((2 * np.pi * Rm / blade["Blade_N"]).to('m').magnitude)  # pitch (KO uses s)

    # ── Flow angles (positive magnitudes) ─────────────────────────────────────
    alpha_1 = float(blade["Beta_1"].to('deg').magnitude)
    alpha_2 = float(blade["Beta_2"].to('deg').magnitude)

    tan_1   = np.tan(np.radians(alpha_1))
    tan_2   = np.tan(np.radians(alpha_2))
    cos_2   = np.cos(np.radians(alpha_2))
    cos_b1k = np.cos(blade["Beta_1"].to('rad').magnitude)

    # KO Eq. 13 — α₂ enters with opposite sign to α₁ for a turbine
    alpha_m = np.degrees(np.arctan(0.5 * (tan_1 + tan_2)))
    cos_m   = np.cos(np.radians(alpha_m))

    # ── Lift coefficient parameter — KO Eq. 12 ────────────────────────────────
    # C_L / (s/c) = 2(tan α₁ + tan α₂) cos α_m
    # s/c here is pitch/true-chord
    C_L_sc  = 2.0 * (tan_1 + tan_2) * cos_m          # = C_L / (s/c)

    # ── Aspect ratio function — KO Eq. 14 ─────────────────────────────────────
    # h/c uses TRUE chord
    h_c = h / c
    if h_c <= 2.0:
        f_AR = (1.0 - 0.25 * np.sqrt(2.0 - h_c)) / h_c
    else:
        f_AR = 1.0 / h_c

    # ── Mach number corrections ────────────────────────────────────────────────
    # K1, K2, Kp — KO Eqs. 7, 8 (same as profile loss)
    K1 = 1.0
    if M_exit > 0.2:
        K1 = 1.0 - 1.25 * (M_exit - 0.2)
    K2 = (M_inlet / M_exit) ** 2
    Kp = 1.0 - K2 * (1.0 - K1)

    # K3, Ks — KO Eq. 15 and Fig. 13
    # K3 = (1 / (h/bx))² where bx is axial chord
    K3  = (bx / h) ** 2
    Ks  = 1.0 - K3 * (1.0 - Kp)

    # ── Y_s,AMDC — KO Eq. 11 ──────────────────────────────────────────────────
    Y_s_AMDC = (0.0334 * f_AR
                * (cos_2 / cos_b1k)
                * C_L_sc ** 2
                * (cos_2 ** 2 / cos_m ** 3))
    ...
    # ── Convert Yp → zeta ─────────────────────────────────────────────────────
    den_conv  = (1.0 + (gamma - 1.0) / 2.0 * M_exit ** 2) ** (gamma / (gamma - 1.0)) - 1.0
    zeta_final = (0.1) * Y_s * (gamma / 2.0 * M_exit ** 2) / den_conv
...
def relative_to_absolute_velocity(W, blade, ang, rpm=Q_(150000,'rpm')):
    omega = rpm.to('rad/s')
    Rm = mean_radius(blade, outlet=True)
    U = omega * Rm
    W_theta = W * np.sin(ang)
    C_theta = W_theta - U
    return C_theta

def absolute_to_relative_velocity(flow, blade, rpm=Q_(150000,'rpm')):
    omega = rpm.to('rad/s')
    Rm = mean_radius(blade, outlet=False)
    U = omega * Rm
    C1 = flow["W"]
    Cm = flow["Cm"]
    alpha_1 = np.acos((Cm/C1)).to('rad')
    C_theta = C1 * np.sin(alpha_1)
    W_theta = C_theta - U
    return W_theta
...
def marsh_zeta(W1, W2, M1_mean, gamma, blade, inlet,
    ...
    """
    Secondary-flow-induced endwall loss after Marsh (1976) / Coull (2017).

    Returns xi_sec = 0.0021 * AF_Marsh  (equation 22 in the paper).

    AF_Marsh is equation (23):
        2 * M* * (V1/V2)^2 * [ dT*·Cx/(s·cos a2)  +  |(V2/V1)·sin a1 - sin a2| / cos a2 ]

    dT* is equation (13):
        integral_PS(V2/Vfs · dS/Cx)  -  integral_SS(V2/Vfs · dS/Cx)
    Note: both integrals normalise arc-length by Cx, NOT by S0.
    """
    ...
    # Flow angles — must carry sign: a1 > 0 (inlet swirl), a2 < 0 (exit swirl)
    a1 = float(np.radians(blade["Beta_1"]))   # positive
    a2 = float(np.radians(blade["Beta_2"]))   # negative for turbine exit

    W2mag = float(W2.to('m/s').magnitude)
    W1mag = float(W1.to('m/s').magnitude)

    # ── Total suction-surface arc length (needed only for suction profile helper) ──
    S0_SS = float(arc_length(blade).to('m').magnitude)   # suction surface only

    # ── Suction surface: Vfs(S) and S values ─────────────────────────────────────
    # suction_profile returns (S_parts, U_parts) in physical units [m] and [m/s]
    S_parts, U_parts = suction_profile(W2mag, blade, DF=DF,
                                       Speak_over_S0=Speak_over_S0, LEI=LEI)
    S_suction = np.concatenate([S_parts[0], S_parts[1]])[1:]
    V_suction = np.concatenate([U_parts[0], U_parts[1]])[1:]

    # Integration variable for both surfaces must be S/Cx
    z_ss = S_suction / Cax                    # dimensionless, ∈ [0, S0_SS/Cax]
    integrand_ss = W2mag / V_suction          # V2 / Vfs  (dimensionless)

    
    # Pressure surface: approximately constant at inlet relative velocity
    V_press      = np.full(n_integ, W1mag)
    S0_PS = S0_SS*0.86
    z_ps         = np.linspace(0.0, S0_PS / Cax, n_integ)
    integrand_ps = W2mag / V_press      # = W2/W1, constant
    T_star_PS    = np.trapezoid(integrand_ps, z_ps)

    # ── ΔT*  (equation 13): T*_PS − T*_SS, should be > 0 ────────────────────────
    T_star_SS = np.trapezoid(integrand_ss, z_ss)
    dT_star   = T_star_PS - T_star_SS   # positive: PS is slower → larger integral

    # ── Compressibility factor M*  (equation 21) ─────────────────────────────────
    M_star = 1.0 + ((gamma - 1.0) / 2.0) * M1_mean**2

    # ── Amplification factor AF_Marsh  (equation 23) ─────────────────────────────
    # Two terms inside the brackets:
    term1 = (dT_star * Cax) / (pitch * np.cos(a2))
    term2 = np.abs((W2mag / W1mag) * np.sin(a1) - np.sin(a2)) / np.cos(a2)

    AF_marsh = 2.0 * M_star * (W1mag / W2mag)**2 * (term1 + term2)

    # ── Secondary loss  (equation 22) ────────────────────────────────────────────
    xi_sec = 0.0021 * AF_marsh

    return xi_sec

def cd_zeta(W1, W2, M1_mean, M2_mean, gamma, blade, inlet,
            Cd=0.002, n_ax=80, n_tan=40,
            DF=0.28, Speak_over_S0=0.52, LEI=0.45):
    ...
    Rm    = float(mean_radius(blade, outlet=True).to('m').magnitude)
    pitch = 2.0 * np.pi * Rm / blade["Blade_N"]
    a2    = float(np.radians(blade["Beta_2"]))

    W2mag = float(W2.to('m/s').magnitude)
    W1mag = float(W1.to('m/s').magnitude)
    g     = gamma

    # ── Suction surface velocity ratios Vfs/V2 ───────────────────────────
    S0_SS    = float(arc_length(blade).to('m').magnitude)
    S_parts, U_parts = suction_profile(W2mag, blade, DF=DF,
                                       Speak_over_S0=Speak_over_S0, LEI=LEI)
    S_ss  = np.concatenate([S_parts[0], S_parts[1]])[1:]   # drop LE stagnation
    V_ss  = np.concatenate([U_parts[0], U_parts[1]])[1:]
    x_ss_norm = S_ss / S0_SS                                # ∈ (0, 1]
    Vr_ss     = V_ss / W2mag                                # Vfs/V2

    # ── Pressure surface velocity ratio ──────────────────────────────────
    # PS runs at approximately inlet velocity throughout (lightly loaded)
    Vr_ps = W1mag / W2mag                                   # Vps/V2, scalar

    # ── Uniform axial grid and interpolated Vr ───────────────────────────
    x_ax       = np.linspace(0.0, 1.0, n_ax)
    Vr_ss_grid = np.interp(x_ax, x_ss_norm, Vr_ss,
                           left=Vr_ss[0], right=Vr_ss[-1])
    Vr_ps_grid = np.full(n_ax, Vr_ps)

    # ── Reference isentropic quantity ─────────────────────────────────────
    # T2/Tt from exit Mach; V2²/(2CpTt) = 1 - T2/Tt
    T2_over_Tt      = 1.0 / (1.0 + (g - 1.0) / 2.0 * M2_mean**2)
    V2sq_over_2CpTt = 1.0 - T2_over_Tt

    # ── Double integral over endwall area (eq. 10) ────────────────────────
    eta          = np.linspace(0.0, 1.0, n_tan)
    integrand_2d = np.zeros((n_ax, n_tan))

    for i, (vr_ps, vr_ss) in enumerate(zip(Vr_ps_grid, Vr_ss_grid)):
        Vr = vr_ps + eta * (vr_ss - vr_ps)          # linear tangential sweep

        # Tfs/T2 from energy equation (same Tt throughout)
        # Tfs/T2 = 1 - (Vr² - 1) * V2²/(2CpTt) / (T2/Tt)
        Tfs_over_T2 = 1.0 - (Vr**2 - 1.0) * V2sq_over_2CpTt / T2_over_Tt
        Tfs_over_T2 = np.maximum(Tfs_over_T2, 0.01)  # guard supersonic SS only

        # eq(10): (T2/Tfs)(rho_fs/rho2)(Vfs/V2)^3
        #       = Vr^3 * (Tfs/T2)^(-g/(g-1))
        integrand_2d[i, :] = Vr**3 * Tfs_over_T2 ** (-(g / (g - 1.0)))

    area_integral = np.trapezoid(
                        np.trapezoid(integrand_2d, eta, axis=1),
                        x_ax)

    # ── Geometric prefactor A_ew / (h · s · cos a2) ──────────────────────
    A_endwall   = Cax * pitch
    A_exit      = h * pitch * np.abs(np.cos(a2))
    geom_factor = A_endwall / A_exit

    xi_cd = 2.0 * Cd * geom_factor * area_integral
    
    return xi_cd

 
def zeta_wet(blade, Xus_Cax=0.5, Xds_Cax=0.5):
    Cd = 0.002
    Rm = mean_radius(blade, outlet=True).to('m')
    t = 2*blade["TE_radius"] 
    pitch = ((2 * np.pi * Rm) / blade["Blade_N"]).to('m').magnitude
    span = ((blade["R_o_outlet"] - blade["R_i_outlet"])).to('m').magnitude
    cax = blade["Chord_ax"].to('m').magnitude
    
    S = arc_length(blade).to('m').magnitude
    
    A_blade = 2 * S * span
    A_endwall = 2 * (Xus_Cax + Xds_Cax) * cax * pitch
    A_wet = A_blade + A_endwall
    
    cos_b2 = np.cos(blade["Beta_2"].to('rad').magnitude)
    return (Cd * A_wet) / (pitch * span * cos_b2) # Strictly a float now
...
def deviation(blade, lam, lam_crit):
    Rm    = mean_radius(blade, outlet=True)
...
def sup_deviation(blade, lam_out, Pt_mag, Tt_mag, gamma, R_mag, n_poly, mdot_choke=None):
    """
    Supersonic exit flow angle from mass conservation.
    Uses isentropic critical lambda (lam_crit=1.0 in reduced coords).
    
    Without mdot_choke: cos(alpha) = cos(beta2_metal) * q(lam_crit)/q(lam_out)
    With mdot_choke:    cos(alpha) = mdot*sqrt(Tt) / (mf_term * Pt * A_exit * q(lam_out))
    """
    gp1      = gamma + 1.0
    gm1      = gamma - 1.0
    ...
    alpha = deviation(blade, lam_crit,lam_crit)
    cos_b = np.cos(alpha)
    A_out_nb    = float(axial_exit_area_nb(blade).to('m**2').magnitude)
    #A_out    = float(axial_exit_area(blade).to('m**2').magnitude)
...
def blade_passage_new(blade, input, gas, P_out, r_loss, rpm=Q_(0,'rpm'), mdot_choke=None):
    ...
    for _ in range(30):
        gp1_gm1 = (gamma_out + 1.0) / (gamma_out - 1.0)

        # n_poly from polytropic relation between inlet total and exit static
    ...
    # ── Choke detection — computed after loop, not inside ─────────────────
    lam_crit = lam_crit_polytropic(n_poly, gamma)  
    ...
    # ── Outlet flow angle ─────────────────────────────────────────────────
    if not choked:
        alpha_out = deviation(blade, lam_out, lam_crit)
    else:
    ...
    return {
        ...
        "alpha_out": alpha_out,
        "mdot":     Q_(mdot, 'kg/s'),
        "n_poly":   gamma_out,  # isentropic — n_poly = gamma
    }
...
def compute_blade_losses(blade, inlet, outlet, gas, rpm=Q_(150000,'rpm')):
    ...
    # ── Profile loss ──────────────────────────────────────────────────────
    zeta_s = profile_loss(W1, W2, blade, mu_out, rho_out, gamma_out, M_w1, M_w2, Ps1, Ps2, inlet, outlet)
